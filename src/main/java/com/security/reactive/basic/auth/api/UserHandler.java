...
public class UserHandler {
	...
	public Mono<ServerResponse> getAllUsers(ServerRequest request) {
		return ok().contentType(MediaType.APPLICATION_JSON)
				.body(userService.findAll()
						.map(userResourceAssembler::toResource), UserResource.class);
	}

	public Mono<ServerResponse> getUser(ServerRequest request) {
		return userService.findById(UUID.fromString(request.pathVariable("userId")))
				.map(userResourceAssembler::toResource)
				.flatMap(ur -> ok().contentType(MediaType.APPLICATION_JSON)
	...
	public Mono<ServerResponse> createUser(ServerRequest request) {
		return ok().build(
				userService.create(request.bodyToMono(CreateUserResource.class)
